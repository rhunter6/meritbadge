...
class clients
{
    ...
    {
        if ($data['badge_id'] != 0) {$badge_search = "AND b.t_badge_id = '$data[badge_id]'";} else { $badge_search = "";}
        if ($data['troop_id'] != "") {$troop_search = "AND m.troop_id like '%$data[troop_id]'";} else { $troop_search = "";}
        if ($data['council_id'] == "YES") {$council_search = "AND m.council = 'YES'";} else { $council_search = "";}
        if ($data['district_id'] != "") {$district_search = "AND m.district_id like '%$data[district_id]%'";} 
    ...
    {
        if ($data['badge_id'] != 0) {$badge_search = "AND b.t_badge_id = '$data[badge_id]'";} else { $badge_search = "";}
        if ($data['troop_id'] != "") {$troop_search = "m.troop_id = '$data[troop_id]' AND ";} else { $troop_search = "";}
        if ($data['council_id'] == "YES") {$council_search = "OR m.council = 'YES'";} else { $council_search = "";}

        if ($data['district_id'] != "") {$district_search = "AND ( ( $troop_search m.district_id = '$data[district_id]'  )  $council_search  ) ";} else { $district_search = "";}
        if (($data['limit1'] >= 0) && ($data['limit2'] != 0)) {
            $limit = " LIMIT " . $data['limit1'] . ", " . $data['limit2'];
        } else {
            $limit = "";
