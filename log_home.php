...
$form_badge = "0";
if (isset($_GET['badge_select'])) {
    $form_badge = $_GET['badge_select'];
}
$district = "";
...
if ($view_list != "") {
    foreach ($view_list as $k => $v) {
        ...
        foreach ($badge_train as $k2 => $v2) {
            $badges .= "$v2[badge_name]\n";
        }
        $badges .= "</textarea>";
        //end badges
        $today = date("Y-m-d");
        if ($v['council'] == "YES") {
            $stamp = "<img src='./img/council.jpg' width='38' height='20' valign='bottom' />";
        } else {
            $stamp = "";
        }
        if ($v['ytp_exp'] <= $today) {
...
foreach ($agentlist as $k => $v) {
    if ($_GET['badge_select'] == $v['badge_id'] and $_GET['badge_select'] != '') {
        $pick_select = "selected ='selected'";
    } else {
        $pick_select = "";
    }
...
function deletePost(list_id,list_name) {
    ...
    if (ask) {
		window.location.href = "./log_home.php?delete_id="+list_id;
